import os
os.environ["MUJOCO_GL"] = "egl"
# os.environ['JAX_PLATFORMS']='cpu'
from minimal_mjx.utils.setupGPU import run_setup
from pathlib import Path
import jax
from jax import numpy as jnp
import numpy as np
import functools
import matplotlib.pyplot as plt
from tqdm import tqdm
import pandas as pd
import argparse
from ral import FINAL_YAMLS, HYPER_TIMES

import moplayground as mop
import minimal_mjx as mm

if __name__ == '__main__':
    run_setup()
    parser = argparse.ArgumentParser()
    parser.add_argument("env", type=str, help="Env to train on")
    args = parser.parse_args()
    config = mm.utils.read_config(FINAL_YAMLS[args.env])
    obj_files = list(Path(config['save_dir'], config['name']).glob('obj*.txt'))
    obj_files.sort(key=lambda x: int(x.name[3:].split('.')[0]))
    if obj_files:
        hvs = []
        nrows, ncols = mm.utils.plotting.get_subplot_grid(len(obj_files))
        fig, axs = plt.subplots(nrows, ncols)
        axs = axs.flatten()
        PLOT_PARETOS = True
        for idx, (file, ax) in enumerate(zip(obj_files, axs)):
            F = pd.read_csv(file).iloc[:, 1:].values
            hv, _ = mop.utils.pareto.get_pareto_statistics(F)
            hvs.append(hv)
            
            if PLOT_PARETOS:
                F_max = F[mop.utils.pareto.get_nondominated(F)]
                ax.scatter(*(F.T), c='black', s=1)
                ax.scatter(*(F_max.T), c='r', s=3)
                ax.set_title(str(file.name))
        
        if PLOT_PARETOS:
            fig.set_size_inches((ncols*3, nrows*3))
            fig.tight_layout()
            fig.savefig(f'ral/plots/{config['env']}-pareto.pdf')
    else:
        rewards_over_iters, directives = mop.eval.pareto.run_experiments(
            config          = config,
            rng             = jax.random.PRNGKey(0),
            env             = mop.envs.create_environment(config, for_training=True)[0],
            N_STEPS         = 500,
            NUM_ENVS        = 2**10,
            save_results    = True
        )
        hvs = []
        for F in rewards_over_iters:
            hv, sp = mop.utils.pareto.get_pareto_statistics(np.array(F))
            hvs.append(hv)
    jax_progress = pd.read_csv(Path(config['save_dir']) / config['name'] / 'progress.csv')
    
    hyper_path = HYPER_TIMES[args.env]
    hyperdf = pd.read_csv(hyper_path)
    
    fig, ax = plt.subplots()
    ax.plot(
        jax_progress['times'].values[1:] - jax_progress['times'].values[0],
        hvs,
        color='blue',
        label='MORLaX'
    )
    ax.plot(
        hyperdf['seconds'].values, #- hyperdf['seconds'].values[0],
        hyperdf['hypervolume'],
        color='red',
        label='HYPER-MORL'
    )
    ax.legend()
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Hypervolume')
    fig.suptitle(f'Hypervolume by time: {config['env']}')
    fig.set_size_inches((10, 5))
    fig.tight_layout()
    fig.savefig(f'ral/plots/{config['env']}-hypervolume-speed.pdf')

    max_hyper_hv        = np.max(hyperdf['hypervolume'].values)
    max_morlax_hv       = np.max(hvs)
    hyper_finish_line   = np.argmax(hyperdf['hypervolume'].values)
    hyper_idx_finish    = np.argmax(hyperdf['hypervolume'].values >= max_hyper_hv)
    morlax_idx_finish   = np.argmax(np.array(hvs) >= max_hyper_hv)
    MORLaX_duration     = jax_progress['times'].iloc[morlax_idx_finish] - jax_progress['times'].iloc[0]
    HYPER_duration      = hyperdf['seconds'].iloc[hyper_idx_finish]
    print('MORLaX finish time', MORLaX_duration)
    print('HYPER-MORL finish time', HYPER_duration)
    print('Speedup factor:', HYPER_duration / MORLaX_duration)
    print('---')
    print('Max hypervolume (HYPER-MORL):', f'{max_hyper_hv:.2e}')
    print('Max hypervolume (MORLaX):', f'{max_morlax_hv:.2e}')
    improvement_ratio = max_morlax_hv / max_hyper_hv
    print('MORLaX improvement over HYPER-MORL:', f'{improvement_ratio:.2f}x')
